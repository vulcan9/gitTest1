...
(function()
{
	...
	_.every = _.all = function(obj, iterator, context)
	{
		...
		each(
				...
				{
					if (!(result = result
							&& iterator.call(context, value, index, list))) return breaker;
				});
		return !!result;
	};
	...
	_.contains = _.include = function(obj, target)
	{
		...
		if (nativeIndexOf && obj.indexOf === nativeIndexOf) return obj
				.indexOf(target) != -1;
	...
	_.invoke = function(obj, method)
	{
		...
		{
			return (_.isFunction(method) ? method : value[method]).apply(value,
					args);
		});
	};
	...
	_.max = function(obj, iterator, context)
	{
		if (!iterator && _.isArray(obj) && obj[0] === +obj[0]
				&& obj.length < 65535) { return Math.max.apply(Math, obj); }
	...
	{
		if (!iterator && _.isArray(obj) && obj[0] === +obj[0]
				&& obj.length < 65535) { return Math.min.apply(Math, obj); }
		if (!iterator && _.isEmpty(obj)) return Infinity;
		var result = {
			computed : Infinity,
			value : Infinity
	...
	{
		return group(obj, value, context,
	...
	_.indexOf = function(array, item, isSorted)
	{
		...
		if (nativeIndexOf && array.indexOf === nativeIndexOf) return array
				.indexOf(item, isSorted);
		for (; i < l; i++)
	...
	{
		var args, bound;
		if (func.bind === nativeBind && nativeBind) return nativeBind.apply(
				func, slice.call(arguments, 1));
		if (!_.isFunction(func)) throw new TypeError;
		args = slice.call(arguments, 2);
		return bound = function()
		{
			if (!(this instanceof bound)) return func.apply(context, args
			...
			for ( var key in a)
			{
				if (_.has(a, key))
				{
					...
					if (!(result = _.has(b, key)
							&& eq(a[key], b[key], aStack, bStack))) break;
				}
			}
